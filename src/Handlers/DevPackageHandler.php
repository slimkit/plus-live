...
use Slimkit\PlusLive\Support\Path;
...
class DevPackageHandler extends \Zhiyi\Plus\Support\PackageHandler
{
    ...
    public function makeMigrationHandle(Command $command)
    {
        ...
        $path = Path::relative(
            $this->app->basePath(),
            $this->app->make('path.plus-live.migrations')
        );
    ...
    {
        $path = $this->app->make('path.plus-live.seeds');
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-live.resource').'/stubs/seeder'
        );
    ...
    public function makeModelHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-live.resources').'/stubs/model'
        );
        $variable = [
            '{--name--}' => $modelName,
            '{--table-name--}' => $table,
